...
const Form = () => {
  ...
  return (
    <form id="contactform" onSubmit={send}>
      {isSending && !anim && (
        <div id="update">
          <p>Остались еще жалобы?</p>
          <button id="btn" onClick={reverseAnim}>
            еще раз
          </button>
        </div>
      )}
      <h1 className="inp">Оставить жалобу можно тут</h1>
      <textarea onInput={handleComplaint} className="inp" placeholder="Message"  />

      <button
      className='complaint__btn'
        id="submit"
        type="submit"
